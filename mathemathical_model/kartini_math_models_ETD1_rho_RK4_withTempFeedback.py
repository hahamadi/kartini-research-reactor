...
def fung_rho(rho_n, t_n, x_n, v, rho_max, H):
    C = (np.pi * rho_max) / (2*H)
...
def fung_rho_update_RK4(rho_n, t_n, dt, x_n, v, rho_max, H):
    ...
    k1 = fung_rho(rho_n, t_n, x_n, v, rho_max, H)
    k2 = fung_rho(rho_n + (dt/2)*k1, t_n + dt/2, x_n, v, rho_max, H)
    k3 = fung_rho(rho_n + (dt/2)*k2, t_n + dt/2, x_n, v, rho_max, H)
    k4 = fung_rho(rho_n + dt*k3, t_n + dt, x_n, v, rho_max, H)
...
t_end = 500 #pos_x_percent/v_percent
#print(t_end)
dt = 0.01      
