import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
from scipy.integrate import solve_ivp
from simulation_config import H, rho_max, v_percent, pos_x_percent, t_end, dt, Lambda
import rho_x_functions as rxf
from simulation_config import beta_groups, lambda_groups, beta

def run_simulation_prke(rho_func, H, rho_max, 
                        v_percent, pos_x_percent, 
                        t_end, dt, 
                        Lambda):
    v_rod = (v_percent / 100.0) * H

    pos_x = (pos_x_percent / 100.0) * H

    times = np.arange(0, t_end + dt, dt)

    def rod_position(t):
        return min(v_rod * t, pos_x)

    def rho_abs_from_t(t):
        return rho_func(rod_position(t)) * beta

    def rhs(t, y):
        n = y[0]
        c = y[1:]

        rho_abs = rho_abs_from_t(t)

        dn = ((rho_abs - beta) / Lambda) * n + np.sum(lambda_groups * c)
        dc = (beta_groups / Lambda) * n - lambda_groups * c

        return np.concatenate(([dn], dc))

    # kondisi awal
    n0 = 1.0
    c0 = beta_groups / (Lambda * lambda_groups) * n0
    y0 = np.concatenate(([n0], c0))

    val = solve_ivp(fun = rhs, 
                t_span=(times[0],times[-1]), 
                y0 = y0, method = "LSODA", 
                t_eval=times)

    n_t = val.y[0, :]
    c_t = val.y[1:, :].T

    pos_t = np.array([rod_position(t) for t in val.t])
    rho_t = np.array([rxf.rho_polynomial(x) for x in pos_t])   # dollar
    rho_abs_t = rho_t * beta

    return pd.DataFrame({
        "time_s" : times,
        "rod_position_m" : pos_t,
        "rod_position_%" : 100 * pos_t / H,
        "rho_dollar" : rho_t,
        "rho_abs" : rho_abs_t,
        "neutron_density_n" : n_t
        })
"""
v_rod = (v_percent / 100.0) * H

pos_x = (pos_x_percent / 100.0) * H

N = int(np.ceil(t_end / dt)) + 1
times = np.arange(0, t_end + dt, dt)

def rod_position(t):
    return min(v_rod * t, pos_x)

def rho_abs_from_t(t):
    return rxf.rho_polynomial(rod_position(t)) * beta

def rhs(t, y):
    n = y[0]
    c = y[1:]

    rho_abs = rho_abs_from_t(t)

    dn = ((rho_abs - beta) / Lambda) * n + np.sum(lambda_groups * c)
    dc = (beta_groups / Lambda) * n - lambda_groups * c

    return np.concatenate(([dn], dc))

# kondisi awal
n0 = 1.0
c0 = beta_groups / (Lambda * lambda_groups) * n0
y0 = np.concatenate(([n0], c0))

val = solve_ivp(fun = rhs, 
                t_span=(times[0],times[-1]), 
                y0 = y0, method = "LSODA", 
                t_eval=times)

#print("success =", val.success)
#print("message =", val.message)

n_t = val.y[0, :]
c_t = val.y[1:, :].T

pos_t = np.array([rod_position(t) for t in val.t])
rho_t = np.array([rxf.rho_polynomial(x) for x in pos_t])   # dollar
rho_abs_t = rho_t * beta

df_out_lsoda = pd.DataFrame({
    "time_s" : times,
    "rod_position_m" : pos_t,
    "rod_position_%" : 100 * pos_t / H,
    "rho_dollar" : rho_t,
    "rho_abs" : rho_abs_t,
    "neutron_density_n" : n_t
    })
#print(val.t)
#print(val.nfev, len(val.t))
#print(val.njev)
"""