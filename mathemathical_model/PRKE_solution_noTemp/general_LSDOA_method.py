...
def run_simulation_prke( drho_dx_func, H, rho_max, v_percent, pos_x_percent, t_end, dt, 
                        Lambda):
    ...
    def drho_dt(t, rho):
        x = rod_position(t)
        v = rod_velocity(t)
        return drho_dx_func(x, rho, rho_max, H) * v
    ...
    def rho_abs_t_rk4():
        ...
        for i in range(len(times) - 1):
            ...
            k1 = drho_dt(t, rho_i)
            k2 = drho_dt(t + 0.5*dt, rho_i + 0.5*dt*k1)
            k3 = drho_dt(t + 0.5*dt, rho_i + 0.5*dt*k2)
            k4 = drho_dt(t + dt, rho_i + dt*k3)
            rho_t[i + 1] = rho_i + (dt / 6.0) * (k1 + 2*k2 + 2*k3 + k4)
