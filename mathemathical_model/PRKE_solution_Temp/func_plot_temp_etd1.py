...
import drhodx_solution_etd1_temp as dset

import simulation_config as config
import rho_x_functions as funcs

case ={
       "Polynomial" : pset.prke_solver_temp(config.H, config.rho_max, 
                                                config.v_percent, 
                                                config.pos_x_percent, 
                                                config.t_end, config.dt, 
                                                config.T0),
       "sinkuadrat" : dset.prke_solver_temp_var(config.H, config.rho_max, 
                                                config.v_percent, 
                                                config.pos_x_percent, 
                                                config.t_end, config.dt, 
                                                config.T0, funcs.drho_dx_sin2),
       "gaussian" : dset.prke_solver_temp_var(config.H, config.rho_max, 
                                                config.v_percent, 
                                                config.pos_x_percent, 
                                                config.t_end, config.dt, 
                                                config.T0, funcs.drho_dx_gauss),
       "sigmoid" : dset.prke_solver_temp_var(config.H, config.rho_max, 
                                                config.v_percent, 
                                                config.pos_x_percent, 
                                                config.t_end, config.dt, 
