...
def fung_temp(T_n, P_n, T0, kT, b):
    return kT * P_n + b * (T0)

def fung_temp_update_etd1(T_n, P_n, dt, n_t, T0, kT, bet):
    
    ch = np.exp(-bet*dt)
    yn = (T_n*ch) + (fung_temp(T_n, P_n, T0, kT, bet)*(ch - 1)/-bet)
...
alpha_T_abs_per_K = 1.5e-4   # reaktivitas absolut per K (mulai dari 5e-5 s/d 2e-4)
a_K_per_s_at_n1 = 0.03     # K/s saat n=1 (pemanasan)
b_1_per_s = 0.01
kT = 0.0003   # K/s per watt
...
for i in range(1, len(times)):
    ...
    T[i] = fung_temp_update_etd1(T[i-1], P_t[i-1], delT, n_t[i-1], T0, kT, b_1_per_s)
